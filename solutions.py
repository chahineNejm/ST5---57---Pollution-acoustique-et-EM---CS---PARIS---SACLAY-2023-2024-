...
def _set_linefem_elementary_stiffness(node_coords, area):
    ...
    mat_e = numpy.array([[1.0, -1.0],[-1.0, 1.0]], numpy.float64)
    mat_e /= area
...
def _set_linefem_elementary_mass(node_coords, area):
    ...
    mat_e = numpy.array([[1.0/3, 1.0/6],[1.0/6, 1.0/3]], numpy.float64)
    mat_e *= area
...
def _set_linefem_elementary_term(node_coords, area, f=(1.0, 1.0, 1.0)):
    ...
    f0 = 2*f[0]+f[1]
    f1 =   f[0]+2*f[1]
    vec_e = numpy.array([f0,f1], dtype=numpy.float)
    vec_e *= (L/6)
...
def _plot_contourf_1(nelems, p_elem2nodes, elem2nodes, node_coords, node_data, **kwargs):
